#!/usr/bin/env bash
cat << EOF > /etc/corosync/vm_stack.crm
...
clone o2cb-clone p_o2cb \
	meta globally-unique=false interleave=true target-role=Started
location cli-ban-vm_ipam2-on-virt-cl-drbd-1 vm_ipam2 role=Started -inf: virt-cl-drbd-1
location cli-ban-vm_quartermaster-on-virt-cl-drbd-1 vm_quartermaster role=Started -inf: virt-cl-drbd-1
